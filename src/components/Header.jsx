...
export const Header = () => {
  const menuNames = ["HOME","ABOUT","PROJECTS","EDUCATION","CONTACT"]
  const [bgColor ,setBgColor] = useState("HOME")
  const [open, setOpen] = useState(false)
  return (
    <nav className="">
      {/* For large devices */}
      <div className="hidden sm:flex w-full h-40 bg-black">
        <div className="w-full  flex items-center justify-center  fixed top-10 ">
           <ul className="hidden sm:flex justify-around items-center sm:w-2/3 md:2/3  lg:w-1/2 h-15  rounded-4xl border border-white/20 bg-black/50 backdrop-blur-3xl">
            {menuNames.map((item)=>(
